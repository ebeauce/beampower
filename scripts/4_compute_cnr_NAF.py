...
metadata = read_asdf.get_asdf_metadata(os.path.join(path_data, data_filename))
...
if use_phasenet:
    ...
    detection_traces = np.clip(
        detection_traces,
        np.min(detection_traces, axis=-1, keepdims=True),
        10.0**5.0 * scale,
    )
...
if use_phasenet:
    ...
    weights_phases = np.ones(
        (detection_traces.shape[:-1]) + (2,), dtype=np.float32
    )
    ...
    weights_phases = np.ones(
        detection_traces.shape[:-1] + (len(phases),), dtype=np.float32
    )
    weights_phases[
        :, :2, 0
    ] = 0.0  # P-wave weight = 0 for horizontal components
    weights_phases[:, 2, 1] = 0.0  # S-wave weight = 0 for vertical components
...
# ------------------------------------------------------
print(
    "The seismic wavefield was recorded on {:d} stations and {:d} channels.".format(
        detection_traces.shape[0], detection_traces.shape[1]
    )
)
print(
    "We backproject the wavefield onto {:d} theoretical sources.".format(
        moveouts.shape[0]
    )
)
t1 = give_time()
...
cnr, source_index = bnr.beamformed_nr.composite_network_response(
    detection_traces, moveouts, weights_phases, weights_sources, device=device
)
t2 = give_time()
