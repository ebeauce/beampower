...
def grid_velocity_model(
    ...
    interpolator_P = interp1d(
        np.hstack((model["z_top"], new_depths)),
...
def compute_tts(
    ...
    # ---------------------------------------------
    print(
        "Latitude: {:.2f} - {:.2f}, {:d} points with {:.2f} spacing.".format(
            latitude_min, latitude_max, n_latitudes, d_latitude
        )
    )
    print(
        "Longitude: {:.2f} - {:.2f}, {:d} points with {:.2f} spacing.".format(
            longitude_min, longitude_max, n_longitudes, d_longitude
        )
    )
    print(
        "Depth: {:.2f} km - {:.2f} km, {:d} points with {:.2f} km spacing.".format(
            depth_min, depth_max, n_depths, d_depth
        )
    )
    ...
    for s, (row_idx, network_sta) in enumerate(network_metadata.iterrows()):
        ...
        for phase in ["P", "S"]:
            ...
            source_depth = (
                -1.0 * network_sta.elevation
            ) / 1000.0  # elevation = -depth, and convert to km
            ...
            print(
                "Spherical coordinates of the source: {:.2f} km, {:.2f} polar angle, {:.2f} azimuthal angle".format(
                    rho_source, theta_source / dtr, lbd_source / dtr
                )
            )
            ...
            t2 = give_time()
            print(
                f"{t2-t1:.2f}sec to compute the {phase}-wave travel time field "
            ...
            # just checking:
            print(
                f'{tts[f"tt_{phase}"][network_sta.station_code].min():.3f}',
...
if __name__ == "__main__":
    ...
    vel_model_filename = (
        "velocity_model_Karabulut2011.csv"  # cite Karabulut et al. (2011)
    )
    ...
    dep_min, dep_max = -2.0, 30.0  # in km
